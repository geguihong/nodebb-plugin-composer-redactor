...
define('composer/drafts', ['redactor'], function(redactor) {
	...
	drafts.init = function(postContainer, postData) {
		...
		$(window).on('redactor:keyup', function(e, data) {
			resetTimeout();

			saveThrottleId = setTimeout(function() {
				saveDraft(postContainer, postData, data);
			}, 1000);
		});
	};
	...
	function saveDraft(postContainer, postData, code) {
        var raw;

		if (canSave() && postData && postData.save_id) {
			raw = code;
			if (raw.length) {
